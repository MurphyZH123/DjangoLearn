...
class TestNewCoupon:
    @pytest.mark.skip(reason="暂时不需要执行创建旺铺券")
    def test_New_WP(self, open_Background):
        ...
        try:
            ...
            logzeros.info('创建旺铺券用例接口走通')
            logzeros.info(f'该用例的返回参数为{res.json()}')
        except AssertionError as e:
            logzeros.error('用例接口走通')
            logzeros.error(f'该用例的返回参数为{res.json()}')
            raise e

    @pytest.mark.skip(reason="暂时不需要执行创建天猫券")
    def test_New_TM(self, open_Background):
        headers = {"accessToken": open_Background}
        url, method, is_json, payload = data[1]['url'], data[1]['method'], data[1]['is_json'], data[1]['data']
        logzeros.info(data[1])
        res = req.visit(url=url, method=method, data=payload, is_json=is_json, headers=headers)
        # 对接口返回数据进行校验
        try:
            assert res.status_code == 200
            logzeros.info('创建天猫券用例接口走通')
            logzeros.info(f'该用例的返回参数为{res.json()}')
        except AssertionError as e:
    ...
    @pytest.mark.skip(reason="暂时不需要执行创建京东券")
    def test_New_JD(self, open_Background):
        headers = {"accessToken": open_Background}
        url, method, is_json, payload = data[2]['url'], data[2]['method'], data[2]['is_json'], data[2]['data']
        res = req.visit(url=url, method=method, data=payload, is_json=is_json, headers=headers)
        # 对接口返回数据进行校验
        try:
            assert res.status_code == 200
            logzeros.info('创建京东券用例接口走通')
            logzeros.info(f'该用例的返回参数为{res.json()}')
        except AssertionError as e:
            logzeros.error('用例执行失败')
            logzeros.error(f'该用例的返回参数为{res.json()}')
    ...
    @pytest.mark.skip(reason="暂时不需要执行创建满减券-微信代金券")
    def test_New_WXMJ(self, open_Background):
        headers = {"accessToken": open_Background}
        url, method, is_json, payload = data[3]['url'], data[3]['method'], data[3]['is_json'], data[3]['data']
        res = req.visit(url=url, method=method, data=payload, is_json=is_json, headers=headers)
        # 对接口返回数据进行校验
        try:
            assert res.status_code == 200
            logzeros.info('创建满减券-微信代金券用例接口走通')
            logzeros.info(f'该用例的返回参数为{res.json()}')
        except AssertionError as e:
            logzeros.error('用例执行失败')
            logzeros.error(f'该用例的返回参数为{res.json()}')
    ...
    @pytest.mark.skip(reason="暂时不需要执行创建兑换券-微信代金券")
    def test_New_WXDJ(self, open_Background):
        headers = {"accessToken": open_Background}
        url, method, is_json, payload = data[4]['url'], data[4]['method'], data[4]['is_json'], data[4]['data']
        res = req.visit(url=url, method=method, data=payload, is_json=is_json, headers=headers)
        # 对接口返回数据进行校验
        try:
            assert res.status_code == 200
    ...
    @pytest.mark.skip(reason="暂时不需要执行批量创建旺铺、天猫、京东、兑换券、满减券")
    ...
    # @pytest.mark.skip(reason="暂时不需要执行创建售货机券")
    def test_New_SHJ(self, open_Background):
        # generate_coupon_excel()
        headers = {"accessToken": open_Background}
        excel_path1 = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))),
                                   'test_data/want_backend/coupons_code001.xlsx')
        files = [
            ('inputFile',
             ('coupons_code001.xlsx', open(excel_path1, 'rb'), 'application/vnd.openxmlformats-officedocument'
                                                               '.spreadsheetml.sheet'))
        ]
        url, method, is_json, payload = data[5]['url'], data[5]['method'], data[5]['is_json'], data[5]['data']
        res = req.visit(url=url, method=method, data=payload, is_json=is_json, headers=headers, files=files)
        try:
            assert res.status_code == 200
            logzeros.info('创建创建售货机券用例接口走通')
            logzeros.info(f'该用例的返回参数为{res.json()}')
        except AssertionError as e:
            logzeros.error('用例执行失败')
            logzeros.error(f'该用例的返回参数为{res.json()}')
...
if __name__ == "__main__":
    # generate_coupon_excel()
    path = os.path.join(os.path.dirname(__file__), 'test_create_coupon.py')
    pytest.main(['-vs', path])
